import CustodianCertificate from '@/components/certificate/CustodianCertificate';

export default function CustodianCertificatePage() {
  return (
    <CustodianCertificate
      collegeName="G PULLAIAH COLLEGE OF ENGINEERING & TECHNOLOGY"
      aggrigations="(Approved by AICTE | NAAC Accreditation with (A) Grade | Accredited by NBA (CSE, ECE & EEE) | Permanently Affiliated to JNTUA)"
      address="Nandikotkur Road, Venkayapalli (V), Kurnool - 518452, Andhra Pradesh"
      studentName="GOLLA LAKSHMI KANTH"
      fatherName="GOLLA BALEESWARAIAH"
      rollNo="21AT1A3516"
      course="IV B.Tech CSO"
      academicYear="2024 to 2025"
      dob="28/05/2004"
      purpose="enrollment in Passport"
      date="05/11/2024"
    />
  );
}
